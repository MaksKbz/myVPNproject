...
- [ ] Полное E2E-тестирование на реальном устройстве (Алматы / РФ) — ждём
      ...
      v3.7.7 с быстрым memory-watch) — по последним записям
      `mem-watch #N` будет видно, растёт ли потребление памяти/fd прямо
      перед крахом (подтверждает OOM) или остаётся стабильным (тогда
      причина в чём-то другом — race condition в самом C-коде под
      многопоточной нагрузкой, не воспроизведённый синтетическим тестом).
