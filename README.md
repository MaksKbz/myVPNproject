...
- [ ] Полное E2E-тестирование на реальном устройстве (Алматы / РФ) — ждём
      ...
      v3.7.5 с checkpoint-трассировкой) — по последней записанной точке
      будет видно последний живой шаг перед смертью процесса; если
      диагноз SIGKILL подтвердится, следующий шаг — инструкция для
      пользователя по настройке ColorOS Startup Manager/Autostart для
      этого приложения (программно обойти SIGKILL от системы невозможно).
