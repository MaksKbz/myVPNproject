...
- [ ] Полное E2E-тестирование на реальном устройстве (Алматы / РФ) — ждём
      ...
      v3.7.6 с ASN-чекпоинтами) — по последней записанной точке будет
      видно, произошёл ли краш внутри `NetworkProfileDetector.detect()`
      (сетевой запрос к ip-api.com), внутри рестарта движка
      (`ProxyEngine.stop()`/`start()` под реальным трафиком), или это
      всё же не связано с ASN auto-detect вовсе (тогда периодические тики
      `runNativeIdleLoop()` покажут точное время смерти процесса).
