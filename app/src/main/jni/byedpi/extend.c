...
static int set_timeout(int fd, unsigned int s) {
    ...
    if (setsockopt(fd, IPPROTO_TCP, TCP_MAXRT, (char *)&sec, sizeof(sec))) {
...
static struct elem_i *cache_add(
    ...
    struct elem_i *val = mem_add(params.mempool,
                                 (char *)data, cmp_len, sizeof(struct elem_i));
    if (!val) { uniperror("mem_add"); free(data); return 0; }
    val->time = t;
    if (!val->extra && *host) {
...
static int on_socks_conn(struct poolhd *pool, struct eval *val, int t) {
    ...
    if (mod_etype(pool, val, POLLIN)) { uniperror("mod_etype"); return -1; }
...
int socket_mod(int fd) {
    if (params.custom_ttl) {
        if (setttl(fd, params.def_ttl) < 0) { return -1; }
    }
...
static int reconnect(struct poolhd *pool, struct eval *val) {
    ...
    client->round_sent = 0;
    client->part_sent = 0;
    if (connect_hook(pool, client, &val->addr, &on_connect)) { return -1; }
    val->pair = 0;
    del_event(pool, val);
    return 0;
}
...
static bool check_ip(
    ...
    if (mem_get(ipset, data, len * 8)) { return 1; }
    return 0;
}

static bool check_proto_tcp(int proto, const char *buffer, ssize_t n) {
    if (!(proto & ~IS_IPV4))                              { return 1; }
    else if ((proto & IS_HTTP)  && is_http(buffer, n))    { return 1; }
    else if ((proto & IS_HTTPS) && is_tls_chello(buffer, n)) { return 1; }
    return 0;
}

static bool check_l34(struct desync_params *dp, int st,
    ...
    if (dp->proto & IS_IPV4) {
        ...
        if (dst->sa.sa_family != AF_INET &&
                memcmp(&dst->in6.sin6_addr, pat, 12)) { return 0; }
    }
    if (dp->pf[0] && (dst->in.sin_port < dp->pf[0] ||
                      dst->in.sin_port > dp->pf[1])) { return 0; }
...
static void swop_groups(struct desync_params *dpc,
        struct desync_params *dpn) {
    LOG(LOG_S, "swop: %d <-> %d\n", dpc->id, dpn->id);
    ...
    if (dpn->prev) dpn->prev->next = dpn;
    if (dpc->next) dpc->next->prev = dpc;
    if (dpc_cp.next != dpn) {
        dpn->next->prev = dpn;
        dpc->prev->next = dpc;
    } else {
...
static int on_torst(struct poolhd *pool, struct eval *val) {
    if (on_trigger(DETECT_TORST, pool, val, 1) == 0) { return 0; }
    struct linger l = { .l_onoff = 1 };
    if (setsockopt(val->pair->fd, SOL_SOCKET, SO_LINGER,
            (char *)&l, sizeof(l)) < 0) {
...
static int on_fin(struct poolhd *pool, struct eval *val) {
    ...
    if (is_client) { val = val->pair; }
    if (!val || !(val->pair->mark && val->round_count <= 1)) { return -1; }
    if (on_trigger(DETECT_TLS_ERR, pool, val, !is_client) == 0) { return 0; }
    return -1;
}
...
static int on_response(struct poolhd *pool, struct eval *val,
    ...
    for (; dp; dp = dp->next) {
        if (dp->bit & val->pair->dp_mask) { continue; }
        if ((dp->detect & DETECT_HTTP_LOCAT) &&
                is_http_redirect(req, qn, resp, sn)) { break; }
        else if ((dp->detect & DETECT_TLS_ERR) &&
                ((is_tls_chello(req, qn) && !is_tls_shello(resp, sn)) ||
                 neq_tls_sid(req, qn, resp, sn))) { break; }
    }
    if (dp) { return on_trigger(dp->detect, pool, val, 1); }
    return -1;
}

static int try_set_tls_timer(struct poolhd *pool, struct eval *val,
        const char *buff, ssize_t n) {
    ssize_t pos = 0;
    if (params.to_bytes_lim && val->recv_count > params.to_bytes_lim) {
...
static void save_hostname(struct eval *client,
        const char *buffer, ssize_t n) {
    if (client->host) { return; }
    char *host = 0;
    int len = parse_tls(buffer, n, &host);
    if (!len) {
        if (!(len = parse_http(buffer, n, &host, 0))) { return; }
    }
    if (!(client->host = malloc(len))) { return; }
...
static struct desync_params *find_dp(struct eval *client,
    ...
    for (;;dp = dp->next) {
        if (!dp) dp = params.dp;
        if (client->dp_mask == params.dp_full_mask) { break; }
        if (!(dp->bit & client->dp_mask) &&
                (dp == init_dp || !dp->detect ||
                 (client->detect & dp->detect)) &&
                (check_l34(dp, SOCK_STREAM, dst)) &&
                (!dp->proto || !buff ||
                 check_proto_tcp(dp->proto, buff, n)) &&
                (!dp->hosts || !buff ||
                 check_host(dp->hosts, buff, n))) {
            return dp;
        }
...
static int recv_and_connect(struct poolhd *pool, struct eval *val, int t) {
    if (!(val->buff = buff_pop(pool, params.bfsize))) { return -1; }
    val->buff->lock = tcp_recv_hook(pool, val, val->buff);
    if (val->buff->lock <= 0) { return -1; }
...
int connect_hook(struct poolhd *pool, struct eval *val,
    ...
    if (!dp) { LOG(LOG_E, "drop connection\n"); return -1; }
    val->dp = dp;
...
static int setup_conn(struct eval *client,
        const char *buffer, ssize_t n) {
    struct desync_params *dp = find_dp(client, buffer, n,
                                       &client->pair->addr);
    if (!dp) { LOG(LOG_E, "drop connection\n"); return -1; }
    save_hostname(client, buffer, n);
    client->mark = is_tls_chello(buffer, n);
    client->dp = dp;
    if (params.timeout && set_timeout(client->pair->fd, params.timeout)) {
        return -1;
    }
    if (pre_desync(client->pair->fd, client->dp)) { return -1; }
    return 0;
}

static int cancel_setup(struct eval *remote) {
    if (post_desync(remote->fd, remote->pair->dp)) { return -1; }
    return 0;
}
...
static int handle_err(struct poolhd *pool, struct eval *val, int e) {
    ...
    case ETIMEDOUT:
    case EHOSTUNREACH:
        if (val->flag == FLAG_CONN) return on_torst(pool, val);
        else                        return on_fin(pool, val);
    }
    return -1;
}
...
ssize_t tcp_recv_hook(struct poolhd *pool, struct eval *val,
    ...
    if (n < 1) {
        if (!n) { return on_fin(pool, val); }
        if (get_e() == EAGAIN) { return 0; }
        uniperror("recv");
...
ssize_t udp_hook(struct eval *val, char *buffer, ssize_t n,
        const union sockaddr_u *dst) {
    ...
    if (!dp) {
        for (dp = params.dp; ; dp = dp->next) {
            if (!dp) { return -1; }
            if (!dp->detect && check_l34(dp, SOCK_DGRAM, dst)) { break; }
        }
        pair->dp = dp;
    }
...
static int protect(int conn_fd, const char *path) {
    ...
    if (err) { uniperror("connect"); close(fd); return -1; }
    ...
    *((int *)CMSG_DATA(cmsg)) = conn_fd;
    ...
    if (sendmsg(fd, &msg, 0) < 0) { uniperror("sendmsg"); close(fd); return -1; }
    if (recv(fd, buf, 1, 0) < 1) { uniperror("recv"); close(fd); return -1; }
    close(fd);
    return 0;
}
#endif
