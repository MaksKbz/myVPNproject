...
void map_fix(union sockaddr_u *addr, char f6) {
    ...
    if (addr->sa.sa_family == AF_INET && f6) {
        ...
        ipv6m->o16 = 0;
        ipv6m->t16 = 0xffff;
    } else if (!ipv6m->o64 && !ipv6m->o16 && ipv6m->t16 == 0xffff && !f6) {
...
static inline char addr_equ(
    ...
    if (a->sa.sa_family == AF_INET) {
        return *((const uint32_t *)(&a->in.sin_addr)) ==
...
static int resolve(const char *chost, int len, union sockaddr_u *addr, int type) {
    struct addrinfo hints = {0}, *res = 0;
    hints.ai_socktype = type;
    hints.ai_flags = AI_ADDRCONFIG;
    if (!params.resolve) hints.ai_flags |= AI_NUMERICHOST;
...
static int resp_s5_error(int fd, int e) {
    struct s5_rep s5r = { .ver = 0x05, .code = (uint8_t )e, .atp = S_ATP_I4 };
    return send(fd, (char *)&s5r, sizeof(s5r), 0);
}

int resp_error(int fd, int e, int flag) {
    if (flag == FLAG_S4) {
        struct s4_req s4r = { .cmd = e ? S4_ER : S4_OK };
        return send(fd, (char *)&s4r, sizeof(s4r), 0);
    }
    else if (flag == FLAG_S5) {
        switch (unie(e)) {
            case 0: e = S_ER_OK; break;
            case ECONNREFUSED: e = S_ER_CONN; break;
            case EHOSTUNREACH:
            case ETIMEDOUT: e = S_ER_HOST; break;
            case ENETUNREACH: e = S_ER_NET; break;
            default: e = S_ER_GEN;
        }
...
static int s4_get_addr(const char *buff, size_t n, union sockaddr_u *dst) {
    if (n < sizeof(struct s4_req) + 1) { return -1; }
    const struct s4_req *r = (const struct s4_req *)buff;
    if (r->cmd != S_CMD_CONN) { return -1; }
    if (ntohl(r->i4.s_addr) <= 255) {
        if (!params.resolve || buff[n - 1] != 0) { return -1; }
        char *id_end = strchr(buff + sizeof(*r), 0);
        if (!id_end) { return -1; }
        int len = (buff + n - id_end) - 2;
        if (len < 3 || len > 255) { return -1; }
...
static int s5_get_addr(const char *buffer, size_t n,
    ...
    switch (r->atp) {
        case S_ATP_I4:
            ...
            break;
        case S_ATP_ID:
            if (!params.resolve) { return -S_ER_ATP; }
            if (r->dst.id.len < 3 ||
            ...
            break;
        case S_ATP_I6:
            if (!params.ipv6) return -S_ER_ATP;
            else {
...
int s5_set_addr(char *buffer, size_t n,
    ...
    if (n < S_SIZE_I4) { return -1; }
    if (addr->sa.sa_family == AF_INET) {
        if (end) { r = (struct s5_req *)(buffer - S_SIZE_I4); }
        r->atp = S_ATP_I4;
        r->dst.i4.ip = addr->in.sin_addr;
        r->dst.i4.port = addr->in.sin_port;
        return S_SIZE_I4;
    } else {
        if (n < S_SIZE_I6) { return -1; }
        if (end) { r = (struct s5_req *)(buffer - S_SIZE_I6); }
        r->atp = S_ATP_I6;
...
static int http_get_addr(
    ...
    if (host_len < 3 || host_len > 255) { return -1; }
...
int create_conn(struct poolhd *pool, struct eval *val,
...
#ifdef __linux__
    int syn_count = 1;
    if (setsockopt(sfd, IPPROTO_TCP, TCP_SYNCNT,
            (char *)&syn_count, sizeof(syn_count))) {
    ...
    int yes = 1;
    if (params.tfo && setsockopt(sfd, IPPROTO_TCP, TCP_FASTOPEN_CONNECT,
            (char *)&yes, sizeof(yes))) {
...
#endif
        if (c < 0) {
            if (get_e() == EAGAIN || get_e() == EINPROGRESS) break;
            uniperror("accept");
            pool->brk = 1;
            return -1;
        }
...
int on_tunnel(struct poolhd *pool, struct eval *val, int etype) {
    ...
    if (val->buff) {
        ...
        val->buff = 0;
        if (mod_etype(pool, val, POLLIN) || mod_etype(pool, pair, POLLIN)) {
            uniperror("mod_etype");
            return -1;
        }
    }
    ...
    do {
        ...
        if (sn < n || wait) {
            buff->lock = n;
            buff->offset = sn;
            if (mod_etype(pool, val, 0) || mod_etype(pool, pair, !wait ? POLLOUT : 0)) {
                uniperror("mod_etype");
                return -1;
            }
            return 0;
...
int on_udp_tunnel(struct poolhd *pool, struct eval *val, int et) {
    ...
    struct eval *pair = val->flag == FLAG_CONN ? val->pair : val->pair->pair;
    do {
        ...
        if (n < 0) {
            ...
            if (err == EMSGSIZE || err == ECONNREFUSED) continue;
            uniperror("recv udp");
            return -1;
        }
        val->recv_count += n;
        ...
        if (val->flag == FLAG_CONN) {
            if (!val->addr.in.sin_port) {
                if (!addr_equ(&addr, &val->addr)) { return 0; }
            ...
            if (offs < 0 || offs > S_SIZE_I6) { return -1; }
...
static int save_buffer(struct poolhd *pool, struct eval *val,
        struct buffer *buff, ssize_t n) {
    ...
    if (params.auto_reconnect) {
        if (!(val->sq_buff = buff_pop(pool, params.bfsize))) { return -1; }
...
static int handle_s5(struct poolhd *pool, struct eval *val,
    ...
    if (val->flag != FLAG_S5) {
        if (auth_socks5(val->fd, buff->data, n)) { return -1; }
        val->flag = FLAG_S5;
        return 0;
    }
    if (n < S_SIZE_MIN) {
    ...
    switch (r->cmd) {
        case S_CMD_CONN:
            ...
            if (s5e >= 0) { return connect_hook(pool, val, dst, &on_connect); }
            break;
        case S_CMD_AUDP:
            if (params.udp) {
                s5e = s5_get_addr(buff->data, n, dst, SOCK_DGRAM);
                if (s5e >= 0) { return udp_associate(pool, val, dst); }
                break;
            }
    ...
    if (resp_s5_error(val->fd, -s5e) < 0) uniperror("send");
    return 1;
}
...
int on_request(struct poolhd *pool, struct eval *val, int et) {
    ...
    if ((params.mode & MODE_SOCKS5) && *buff->data == S_VER5) {
        if ((error = handle_s5(pool, val, buff, n, &dst)) > 0) { return -1; }
        skip_conn = 1;
    }
        ...
        if (save_buffer(pool, val, buff, n - (req_size - 3))) { return -1; }
    }
    else if ((params.mode & MODE_RAWTLS) && is_tls_chello(buff->data, n)) {
        error = tls_get_addr(buff->data, n, &dst);
        if (!error && save_buffer(pool, val, buff, n)) { return -1; }
    }
    else if ((params.mode & MODE_TCP)) {
        if (save_buffer(pool, val, buff, n)) { return -1; }
    }
    else {
    ...
    if (error) {
        if (resp_error(val->fd, ENOENT, val->flag) < 0) uniperror("send");
        return -1;
    }
    return 0;
}
...
int on_connect(struct poolhd *pool, struct eval *val, int et) {
    ...
    if (et & POLLERR) {
        if (getsockopt(val->fd, SOL_SOCKET, SO_ERROR,
                (char *)&error, &len)) {
        ...
        switch (error) {
            ...
            case ECONNREFUSED:
            case ETIMEDOUT:
                if (on_connerr(pool, val) == 0) { return 0; }
        }
    } else {
    ...
    if (resp_error(val->pair->fd, error, val->pair->flag) < 0) {
        uniperror("send");
        return -1;
    }
    return error ? -1 : 0;
