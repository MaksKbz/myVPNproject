...
struct poolhd *init_pool(int count) {
    ...
    if (!pool) { uniperror("init pool"); return 0; }
    pool->max = count;
    pool->count = 0;
    pool->iters = 0;
#ifndef NOEPOLL
    int efd = epoll_create(count);
    if (efd < 0) { free(pool); return 0; }
    pool->efd = efd;
#endif
...
void del_event(struct poolhd *pool, struct eval *val) {
...
#endif
    if (val->buff) { buff_push(pool, val->buff); val->buff = 0; }
    if (val->sq_buff) { buff_push(pool, val->sq_buff); val->sq_buff = 0; }
#ifndef _WIN32
    if (val->restore_fake) {
...
#endif
    if (val->host) { free(val->host); val->host = 0; }
    close(val->fd);
    val->fd = -1;
    val->mod_iter = pool->iters;
    ...
    if (val->pair) {
        if (val->pair->pair == val) { val->pair->pair = 0; }
...
struct eval *next_event(struct poolhd *pool, int *offs, int *type, int ms) {
    while (1) {
        ...
        *offs = i - 1;
        if (val->mod_iter == pool->iters) { continue; }
...
int mod_etype(struct poolhd *pool, struct eval *val, int type) {
    assert(val->fd > 0);
    struct epoll_event ev = { .events = _POLLDEF | type, .data = {val} };
...
void set_timer(struct poolhd *pool, struct eval *val, long ms) {
    ...
    val->tv_next = next;
    val->tv_prev = prev;
    if (next) { next->tv_prev = val; }
    if (prev) { prev->tv_next = val; }
    if (!pool->tv_start || next == pool->tv_start) { pool->tv_start = val; }
    if (!pool->tv_end   || prev == pool->tv_end)   { pool->tv_end   = val; }
}

void remove_timer(struct poolhd *pool, struct eval *val) {
    if (val->tv_prev) { val->tv_prev->tv_next = val->tv_next; }
    if (val->tv_next) { val->tv_next->tv_prev = val->tv_prev; }
    if (pool->tv_start == val) { pool->tv_start = val->tv_next; }
    if (pool->tv_end   == val) { pool->tv_end   = val->tv_prev; }
    val->tv_ms = 0;
    val->tv_next = 0;
    val->tv_prev = 0;
}
...
struct eval *next_event_tv(struct poolhd *pool, int *offs, int *type) {
    if (!pool->tv_start) { return next_event(pool, offs, type, -1); }
    struct eval *val = 0;
    int ms = pool->tv_start->tv_ms - time_ms();
    if (ms > 0) { val = next_event(pool, offs, type, ms); }
...
struct buffer *buff_pop(struct poolhd *pool, size_t size) {
    ...
    if (buff) {
        ...
        if (!buff) { uniperror("malloc"); return 0; }
...
void buff_push(struct poolhd *pool, struct buffer *buff) {
    if (!buff) { return; }
    if (pool->buff_count >= MAX_BUFF_INP) { free(buff); return; }
    buff->lock = 0;
    buff->offset = 0;
