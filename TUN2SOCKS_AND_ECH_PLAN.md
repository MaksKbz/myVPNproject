...
**Не проверено**: сборка через реальный Android NDK и реальный краш на
устройстве — логика идентична для Linux/Bionic (стандартный POSIX
`sigaction`/`write()`, доступный на Android без изменений), но
подтверждение появится только после того, как пользователь пришлёт
содержимое карточки краша из реального теста.
...
| **CrashLogger — диагностика без adb** | ✅ Реализовано (см. раздел 4) | Ждём текст лога от пользователя для финальной диагностики |
