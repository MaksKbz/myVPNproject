...
**Не проверено**: реальный повторный тест на Android-устройстве — фикс
компилируется и логически идентичен для Linux/Bionic (общий C-код без
platform-specific веток в затронутых функциях), но финальное подтверждение
может дать только пользователь на реальном телефоне.
...
| **Краш при рестарте tun2socks (ASN auto-detect)** | ✅ Исправлено (см. раздел 3), протестировано 5x стресс-тестом | Подтверждение на реальном устройстве |
